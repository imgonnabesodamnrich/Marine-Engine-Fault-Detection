...
船舶柴油机是船舶推进系统的核心，其内部结构复杂，零部件众多。其中，**燃烧室组件（Combustion Chamber Components）**——包括气缸盖、气缸套、活塞等——对发动机功能至关重要。
...
尽管故障诊断技术已有发展，但在实际船舶运营中仍面临巨大挑战。根据论文分析，现有方法主要分为三类，各有优劣（详见论文 Table 1）：

*   **模型驱动方法 (Model-Based):**
    *   *原理:* 基于物理定律和工程知识构建数学模型。
    *   *局限:* 建模过程复杂，难以应对非线性系统和复杂的变量交互，缺乏灵活性。
*   **数据驱动方法 (Data-Driven):**
    *   *原理:* 利用机器学习/深度学习自动学习故障特征（如 CNN, SVM 等）。
    *   *局限:* **严重依赖数据质量和数量**。现实中故障数据极度稀缺，导致模型泛化能力差；且模型往往是**“黑盒”**，难以解释决策背后的物理逻辑。
*   **混合方法 (Hybrid):**
    *   *原理:* 结合物理模型与数据驱动方法。
    *   *局限:* 现有混合方法大多仅将物理模型作为数据生成工具，缺乏对模型决策过程的深入机理分析，导致计算成本高且解释性依然不足。

### 3. 本研究的切入点 (Motivation)
针对上述痛点，本研究（TSRF）旨在解决传统智能诊断方法面临的两大核心挑战：
